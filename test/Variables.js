...
describe("Variables", () => {
  ...
  describe("Example 1", () => {
    it("it has a state variable with a default value", async () => {
      ...
      expect(await contract.name()).to.equal("Example1");
    });
  });

  describe("Example 2", () => {
    it("demonstrates the constructor sets the state", async () => {
      const Contract = await ethers.getContractFactory("Variables2");
      let contract = await Contract.deploy("Example2");
      expect(await contract.name()).to.equal("Example2");
    });
  });
